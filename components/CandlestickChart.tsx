...
type CandlestickChartComponentProps = Omit<
...
const CandlestickChart = ({
  ...
  liveInterval,
  setLiveInterval,
}: CandlestickChartComponentProps) => {
  ...
  const fetchOHLCData = async (selectedPeriod: Period) => {
    try {
      ...
      if (selectedPeriod === "max") {
        try {
          const maxData = await fetcher<OHLCData[]>(`/coins/${coinId}/ohlc`, {
            vs_currency: "usd",
            days: "max",
            precision: "full",
          });
          ...
          if (!(maxError instanceof Error) || !maxError.message.includes("API Error: 401")) {
            throw maxError;
          }
  ...
  useEffect(() => {
    ...
    const convertedToSeconds = ohlcData.map(
      (item) =>
        [
          Math.floor(item[0] / 1000), // convert ms to s
          item[1],
          item[2],
          item[3],
          item[4],
